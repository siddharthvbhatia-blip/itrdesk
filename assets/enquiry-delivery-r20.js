(function(){
  ...
  const NATIVE_PROVIDER='https://formsubmit.co/siddharth.v.bhatia@gmail.com';
  ...
  async function postNativeProvider(payload){
    ...
    Object.entries(fields).forEach(([name,value])=>addField(form,name,value));
    ...
    return new Promise((resolve,reject)=>{
      ...
      const succeed=()=>{
        ...
        settled=true;
        cleanup();
        resolve({accepted:true,reference,secondary:true});
      };
      const fail=()=>{
        if(settled)return;
        settled=true;
  ...
  async function submitReliably(payload){
    try{return await postBackendWithRetry(payload)}
      ...
      try{return await postNativeProvider(payload)}
      catch(nativeError){
        nativeError.backendError=backendError;
        throw nativeError;
      }
    }
  }
  ...
  function setup(){
    ...
    if(!form||form.dataset.reliableEnquiry==='r20')return;
    form.dataset.reliableEnquiry='r20';
